...
```bash
# NPM
npm install --save-dev @applaud/webpack-bundle-analyzer
# Yarn
yarn add -D @applaud/webpack-bundle-analyzer
```

<h2 align="center">Usage (as a plugin)</h2>

```js
const BundleAnalyzerPlugin = require('@applaud/webpack-bundle-analyzer').BundleAnalyzerPlugin;
...
If you're on Windows and using PowerShell, you can generate the stats file with this command to [avoid BOM issues](https://github.com/webpack-contrib/@applaud/webpack-bundle-analyzer/issues/47):
...
```
@applaud/webpack-bundle-analyzer bundle/output/path/stats.json
```

<h2 align="center">Options (for CLI)</h2>

```bash
@applaud/webpack-bundle-analyzer <bundleStatsFile> [bundleDir] [options]
```
...
@applaud/webpack-bundle-analyzer reports three values for sizes. `defaultSizes` can be used to control which of these is shown by default. The different reported sizes are:
...
It happens when `@applaud/webpack-bundle-analyzer` analyzes files that don't actually exist in your file system, for example when you work with `webpack-dev-server` that keeps all the files in RAM. If you use `@applaud/webpack-bundle-analyzer` as a plugin you won't get any errors, however if you run it via CLI you get the error message in terminal:
```
Couldn't parse bundle asset "your_bundle_name.bundle.js".
Analyzer will use module sizes from stats file.
```
To get more information about it you can read [issue #147](https://github.com/webpack-contrib/@applaud/webpack-bundle-analyzer/issues/147).
...
[downloads-url]: https://npmjs.com/package/@applaud/webpack-bundle-analyzer
